...
{   
    printk("(open)Hello this is \"open\"(demo_open)\n");
    return 0; 
} 
//关闭设备
static int demo_release(struct inode *inode, struct file *filp) 
{
    printk("(close)Hello this is \"close\"(demo_release)\n");
    return 0; 
}
//读设备
static ssize_t demo_read(struct file *filp, char __user *buf, size_t size, loff_t *offset) 
{
    printk("(read)Hello this is \"read\"(demo_read)\n");
    return 0; 
}
//写设备 
static ssize_t demo_write(struct file *filp, const char __user *buf, size_t size, loff_t *offset)
{
    printk("(write)Hello this is \"write\"(demo_write)\n");
    return 0;
} 
//操作方法集 
...
{
    //get command and pid
    printk("(exit)Hello this is \"exit\"(demo_exit)\n");
